List score_cpp(arma::mat ipar,
  ...
  int ni = ipar.n_rows;
  arma::colvec pos_ths = th ; arma::colvec pre_ths ;
  int iter = 0 ;
  bool converged = false ;
  arma::mat delta ; arma::mat abs_delta ;
  arma::vec theta; arma::vec SE; arma::vec d2_diag ; arma::mat d2_inv ;
  bool conv_status ;
  ...
  while ((iter < maxIter) & (converged == false)) {
    ...
    return List::create(Named("theta")=theta,Named("SE")=SE,
                        Named("Hessian")=deriv2,Named("niter")=iter);
